...
module.exports = {
    ...
    run: async(bot, message, args) => {
        ...
        const character = await characterModel.find({ owners: { guild: message.guild.id, owner: message.member.id }, name: giftingCharacters }).collation({ locale: 'en', strength: 2 });
        ...
        if(nagiftingCharactersme.length > existsList.length) {
            const embed = new MessageEmbed()
                .setColor('EED202')
        ...
        if(giftingCharacters.length > characterList.length) {
            ...
            character.forEach(async(char) => await characterModel.updateMany({ 'owners.guild': message.guild.id, 'owners.owner': message.member.id, id: char.Id }, { $set: { 'owners.$.owner': member.id }}))
