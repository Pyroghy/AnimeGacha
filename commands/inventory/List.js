...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        const character = await characterModel.find({ [`owners.${message.guild.id}`]: member.id }).sort({ 'series.title': 1, name: 1 });
        ...
        message.channel.send({ embeds: [embed], components: pages }).then(async (message) => {
            ...
            buttonCollector.on('collect', async (button) => {
                ...
                embed.setColor(userProfile.guilds[message.guild.id].Color)
                ...
                embed.setThumbnail(userProfile.guilds[message.guild.id].image)
