...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        const waifu = await characterModel.aggregate([{ $match: { [`owners.${message.guild.id}`]: null, gender: 'Female' } }, { $sample: { size: 1 } }]);
            ...
            message.channel.send({ embeds: [embed] }).then(message => {
                ...
                reactionCollector.on('collect', async (reaction, user) => {
                    ...
                    if (cooldown) {
                        ...
                        const claim = await characterModel.updateOne({ [`owners.${message.guild.id}`]: null, id: waifu[0].id }, { $set: { [`owners.${message.guild.id}`]: user.id } });
