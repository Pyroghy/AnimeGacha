...
module.exports = {
    ...
    run: async(bot, message, args) => {
        ...
        const husbando = await characterModel.aggregate([{ $match: { owners: { guild: message.guild.id, owner: 'null' }, gender: 'Male' }}, { $sample: { size: 1 }}]);
            ...
            message.channel.send({ embeds: [embed] }).then(message => {
                ...
                reactionCollector.on('collect', async(reaction, user) => {
                    ...
                    if(cooldown) {
                        ...
                        const claim = await characterModel.updateOne({ 'owners.guild': message.guild.id, 'owners.owner': 'null', id: husbando[0].id }, { $set: { 'owners.$.owner': user.id }});
