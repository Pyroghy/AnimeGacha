...
module.exports.run = async(bot, message, args) => {
    ...
    const Husbando = await CharacterModel.aggregate([{ $match: { owners: { $elemMatch: { guild: message.guild.id, owner: 'null' }}, gender: 'Male' }}, { $sample: { size: 1 }}]);
        ...
        message.channel.send(embed).then(message => {
            ...
            collector.on('collect', async(reaction, user) => {
                ...
                if(cooldown) {
                    ...
                    const Claim = await CharacterModel.updateOne({ owners: { guild: message.guild.id, owner: 'null' }, id: Husbando[0].id }, { $set: { 'owners.$.owner': user.id }});
